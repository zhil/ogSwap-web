<template>
  <div class="w-[167px] relative">
    <btn
      ...
      block
      :readonly="signed"
      class="h-[40px] leading-[40px] relative"
    ...
    >
      ...
      />
      <span
        class="absolute text-xs leading-none transition-all"
        :class="{
          'left-10 font-semibold top-2': signed && network,
          'left-10 font-semibold top-3.5': signed && !network,
          'left-[57px] top-3.5': !signed,
        }"
        >{{ signed ? formattedAddress : 'Connect' }}</span
      >
      <span
        v-show="network"
        class="absolute left-10 bottom-2 text-[10px] font-normal leading-none"
        :class="{ 'pointer-events-none opacity-0': !signed }"
      >
        <span
          class="w-[7px] h-[7px] inline-block rounded-full transition"
          :class="{
            'bg-[#07DA9E]': connected,
...
export default Vue.extend({
  ...
  computed: {
    formattedAddress(): string {
      ...
      return (
        this.address.slice(0, 4) + '...' + this.address.slice(len - 5)
      )
    },
  },
})
